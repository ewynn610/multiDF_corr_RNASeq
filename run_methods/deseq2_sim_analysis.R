...
##
## Purpose of script: Analyze simulated data using DESeq2 (ignoring correlation)
##
...
reduced_design_mats$c1=model.matrix(~time, data=meta_data)

## Difference between any timepoints in treatment group
design_mat_c2=model.matrix(~group+group:time, data=meta_data)
## Remove group 1 interactions
reduced_design_mats$c2=design_mat_c2[,-grep("group1:", colnames(design_mat_c2))]

## Are any of the interactions significant
reduced_design_mats$c3=model.matrix(~group+time,data=meta_data)

## Any Significant Coefficients
reduced_design_mats$c4=model.matrix(~1, data=meta_data)

############## Run tests with reduced matrices #################
lrt_res=lapply(reduced_design_mats, function(x){
  df=data.frame(results(DESeq(dds, test='LRT', reduced = x), 
                     independentFiltering=F, cooksCutoff=FALSE))
